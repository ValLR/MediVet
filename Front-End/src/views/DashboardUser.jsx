...
/**
 * Componente DashboardUser: Vista principal para los Dueños de mascotas.
 * Muestra el saludo personalizado, las próximas citas agendadas por el dueño y un widget
...
const DashboardUser = () => {
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      const nameParts = userObj.nombre_completo ? userObj.nombre_completo.split(' ') : ['Usuario'];
          ...
          {isLoading ? (
            ...
            <ul className="list-disc pl-10 space-y-2 text-lg text-[#0c3a6f] font-medium">
              {citas.map((cita) => {
                ...
                let docNombre = 'Desconocido';
                if (cita.veterinario_detalle) {
                  const vet = cita.veterinario_detalle;
                  const fullname = `${vet.first_name || ''} ${vet.last_name || ''}`.trim();
                  docNombre = fullname ? fullname : vet.username;
                }

                return (
                  <li key={cita.id}>
                    <span className="text-black font-normal">
                      {time} - {mascotaNombre} (Doctor: {docNombre}).
                    </span>
                  </li>
                );
              })}
